...
The `Logger` interface itself is frozen (defined in `interfaces-runtime.md` line 127–136) — these implementations must conform exactly to it. No changes to the interface are permitted.
...
| Fix         | Create `json-logger.ts`, `pretty-logger.ts`, and barrel export in `packages/core/src/logger/`.                              |
...
| Fix         | Add exports for both classes.                                         |
...
Default `output`: writes via `console.log` for non-error levels and `console.error` for error level. This is runtime-agnostic — `console` is available in Node.js, browsers, Deno, and Bun.
...
```
{"timestamp":"2026-06-21T10:30:00.000Z","level":"info","msg":"Run started","meta":{"runId":"abc-123"}}
{"timestamp":"2026-06-21T10:30:00.001Z","level":"debug","msg":"Generating","meta":{"runId":"abc-123","messageCount":5}}
```
...
- Level: padded to 5 characters for alignment.
- Meta fields: rendered as `key: value` pairs in parentheses.
- Colors (ANSI): Applied only when `output` is connected to a TTY. Default detection via `typeof process !== 'undefined' && process.stdout?.isTTY`. This is the only runtime check — acceptable because it's a single detection at construction time, not per-call.
- Color scheme:
  - `error`: red
  - `warn`: yellow
  - `info`: green
...
- Do NOT use `process.stdout` directly — use `console.log` / `console.error` via the pluggable `output` function.
...
- Do NOT use `lib: ["DOM"]` — the project targets `lib: ["ES2022"]`. The color detection uses a guarded `typeof process !== 'undefined'` check.
...
| `packages/core/src/index.ts`                | Modify | Add exports for `JsonLogger` and `PrettyLogger` |
 ...
 * // {"timestamp":"2026-06-21T10:30:00.000Z","level":"info","msg":"Run started","meta":{"runId":"abc"}}
 */
...
export class PrettyLogger implements Logger {
  ...
  constructor(options: PrettyLoggerOptions = {}) {
    this.level = LOG_LEVELS[options.level ?? 'debug'];
    this.output = options.output ?? ((line: string) => console.log(line));
    this.useColors = options.colors ?? hasTTY();
  }
...
export { JsonLogger } from './logger/json-logger.js';
export { PrettyLogger } from './logger/pretty-logger.js';
```

Note: The options types (`JsonLoggerOptions`, `PrettyLoggerOptions`) are intentionally NOT exported from the top-level `index.ts` — they are internal construction details. Users who need full type access can import from the specific module path if needed.
...
16. All existing tests pass (orchestrator uses no-op logger — unchanged).
17. TypeScript compilation succeeds with `lib: ["ES2022"]` (no DOM or Node.js types needed).
...
| ------------------------------------------------------------ | ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `console.log` output in non-Node runtimes differs            | LOW        | LOW    | `console.log` is defined in the Web API spec and available in all modern runtimes. Browsers, Deno, and Bun all support it.                      |
| ANSI color codes in non-TTY output (CI, logs)                | LOW        | MEDIUM | Auto-detection via `process.stdout.isTTY` guards against this. Users can explicitly set `colors: false` to override.                            |
...
- `.opencode/skill/interfaces/SKILL.md` — `Logger` interface (§Logger Contract, lines 127–136)
